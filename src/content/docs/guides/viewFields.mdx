...
Note that the `context.bound` object is read-only.
If you want to change the value of a bind target from within the JS View Field, you need to either use the `save to` bind target or the [`setMetadata` API method](/obsidian-meta-bind-plugin-docs/api/classes/obsapi/#setmetadata).

The JS View Field will automatically update when one of the bound variables of the bind target sections changes.
This also means that input fields rendered by the JS View Field will lose focus when one of the JS View Field's bound variables changes.
...
:::tip
You can find more examples [here](/obsidian-meta-bind-plugin-docs/guides/advancedusecases/).
:::
