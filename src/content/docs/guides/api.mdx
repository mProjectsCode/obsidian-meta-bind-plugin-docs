...
Most things that need lifecycle management inherit from [`Mountable`](/obsidian-meta-bind-plugin-docs/api/classes/mountable), which has a `mount` and `unmount` method.
...
which mounts and unmounts the mountable on load and unload respectively, then it registers that [`MarkdownRenderChild`](https://docs.obsidian.md/Reference/TypeScript+API/MarkdownRenderChild) as a child of the passed in component.
Note that this just defers the lifecycle management to the [component](https://docs.obsidian.md/Reference/TypeScript+API/Component), which is the primitive that Obsidian uses to do lifecycle management.
You still need to make sure that the [component](https://docs.obsidian.md/Reference/TypeScript+API/Component) is correctly loaded and unloaded.
...
Note that you still need to make sure that the [component](https://docs.obsidian.md/Reference/TypeScript+API/Component) is correctly loaded and unloaded.
...
When using [JS Engine](https://github.com/mProjectsCode/obsidian-js-engine-plugin), in most cases you should be using the global `container` and `component` variables provided by [JS Engine](https://github.com/mProjectsCode/obsidian-js-engine-plugin).
...
- `container` - The container for the rendered content. This is of type `HTMLElement`. Only available in a rendering context.
...
const buttonOptions = {
	...
	isPreview: false, // This flag is used to render a non-functional preview of the button. Internally this is used for the Button Builder.
};
